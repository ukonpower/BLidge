...
### エクスポートフロー
1. **WebSocket同期**: リアルタイムでブラウザとBlenderを同期
2. **シーンエクスポート**: 完全なシーンデータをJSON化
3. **glTFエクスポート**: 標準フォーマットでメッシュ/アニメーションを出力

### データ変換
- **座標系**: Blender (Y-up) → カスタム (Z-up) に変換
- **メッシュデータ**: Base64エンコードで効率的に転送
- **アニメーション**: ベジエハンドル情報を含む完全なキーフレームデータ

### アクセサーシステム
- F-Curveに一意のアクセサーIDを割り当て
